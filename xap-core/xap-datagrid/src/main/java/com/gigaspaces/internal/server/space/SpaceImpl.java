...
public class SpaceImpl extends AbstractService implements IRemoteSpace, IInternalRemoteJSpaceAdmin, DiscoveryListener,
    ...
    public SpaceImpl(String spaceName, String containerName, JSpaceContainerImpl container, SpaceURL url,
        ...
        if (SystemInfo.singleton().getManagerClusterInfo().getManagerClusterType().equals(ManagerClusterType.SERVICE_GRID)
                && !SystemInfo.singleton().getManagerClusterInfo().getManagers().isEmpty()
                && _jspaceAttr.isTieredStorageCachePolicy() && !hasInstanceLevelSla
                && !Boolean.parseBoolean(customProperties.getProperty(Constants.TieredStorage.AUTO_GENERATE_SLA_PROP))) {
            throw new CreateException("Could Not create space " + spaceName + ", no per instance host sla and context property pu.autogenerated-instance-sla wasn't set to true");
        }
        if (_scalable) {
